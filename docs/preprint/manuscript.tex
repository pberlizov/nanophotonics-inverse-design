...
\begin{abstract}
We study a reproducible search \emph{pattern} for \textbf{DRC-manifold-feasible} silicon 1$\times$2 splitters: sample on a generative manifold, rank with a lightweight surrogate, and promote only MEEP-verified layouts under a frozen forward-model contract (\texttt{phase0\_v1}).
Here ``DRC-feasible'' means feasible under the \texttt{drcgenerator} EBeamModel generative rules, not foundry sign-off DRC.
...
We claim:
\begin{enumerate}
  \item A reproducible MEEP-gated loop on a generator-rule-feasible manifold: sample $\to$ surrogate rank $\to$ MEEP verify $\to$ corpus, with open artifacts at \url{https://github.com/pberlizov/nanophotonics-inverse-design}.
...
Figure~\ref{fig:pipeline} sketches the overall loop: sample on the DRC-manifold-feasible search space, rank with a surrogate, verify shortlisted candidates in MEEP, and append verified labels to the corpus.
...
Each latent vector $\mathbf{z}$ decodes to a binary 180$\times$180 mask satisfying \emph{generator-rule} width/spacing constraints on the training manifold.
\textbf{DRC-manifold-feasible} in this paper means passing those generative rules only; it is \emph{not} a foundry rule-deck sign-off.
We do not run end-to-end differentiable manifold optimization as in Danis \emph{et al.}; instead, we borrow their decoder as the feasible search space, ensuring that every candidate mask satisfies the model's heuristic rules before simulation.
...
Training uses MEEP labels from \texttt{sim\_results\_phase0\_v1\_all.csv} (corpus snapshots differ by row count and source filter).
Negative holdout $R^2$ is expected: most labels lie far from 50/50, so absolute error regression is ill-conditioned.
The model's job is instead to sort $M \gg B$ proposals cheaply and let MEEP spend its budget on the top $B$; it never promotes a design on its own.

Table~\ref{tab:surrogate_snapshots} collects surrogate metrics that are easy to confuse across experiments.
Rows differ by \emph{corpus snapshot}, \emph{train/val split}, \emph{loss mode}, and \emph{evaluation protocol}---not by accidental contradiction.
The production shortlist model for \texttt{round\_rank} is \texttt{surrogate\_improved\_rank} (pairwise rank loss).
A separate MSE baseline (\texttt{surrogate\_improved}) achieves higher top-20 in-spec counts on the same perturb+search holdout but is not the model deployed for MEEP-gated rounds.
Proposal-pool ablation (row~5) scores \emph{fraction in-spec} within a fixed $n{=}500$ label pool with fast subsampling---a different statistic from holdout top-20 counts.

\begin{table}[t]
  \centering
  \caption{Surrogate validation snapshots (different corpora, splits, and metrics).}
  \label{tab:surrogate_snapshots}
  \small
  \begin{tabular}{p{2.1cm}p{1.5cm}p{0.9cm}rrrrp{1.1cm}p{1.4cm}}
    \toprule
    Snapshot & Split & Loss & $n_{\mathrm{tr}}$ & $n_{\mathrm{val}}$ & $R^2$ & $\rho$ & top-$k$ in-spec & Artifact \\
    \midrule
    MSE improved holdout & 80/20 rand. & MSE & 2956 & 739 & $-0.51$ & 0.38 & 13 / 1 & \texttt{ranking\_eval\_improved.json} \\
    Rank vs.\ MSE cmp. & same eval & MSE / rank & --- & --- & --- & 0.71 / 0.73 & 13 / 5 vs.\ 1 & \texttt{rank\_loss\_comparison.json} \\
    Prod.\ rank holdout & 80/20 rand. & rank & 2610 & 653 & $-19$ & 0.50 & 5 / 1$^\dagger$ & \texttt{surrogate\_improved\_rank} \\
    Merged full eval & holdout & rank & --- & 3191 & --- & 0.64 & 8 / 3 & \texttt{ranking\_eval\_merged\_full.json} \\
    Proposal pool $K{=}20$ & pool $n{=}500$ & rank & --- & --- & --- & --- & 15\% / 10\% & \texttt{ablation\_proposal\_pool.json} \\
    Val.\ random 80/20 & 80/20 rand. & rank & 2768 & 692 & $-9.1$ & 0.40 & 7 / 20 & \texttt{surrogate\_validation.json} \\
    Val.\ $\sigma$-group & $\sigma$ groups & rank & 3245 & 215 & $-11.1$ & 0.46 & 8 / 20 & idem \\
    Val.\ chronological & time split & rank & 301 & 25 & $-89$ & 0.08 & 6 / 6 & idem \\
    \bottomrule
  \end{tabular}
  \\[0.3em]
  {\footnotesize $R^2$ and $\rho$ are on $|R_{\mathrm{up}}-0.5|$ unless noted. top-$k$ in-spec: surrogate top-20 vs.\ random top-20 (rows 1--4, 6--8) or fraction in-spec (row 5). $^\dagger$Random baseline 1 from row~1 shared holdout.}
\end{table}
...
On the shared perturb+search corpus (row~2 vs.\ row~3 in Table~\ref{tab:surrogate_snapshots}), pairwise rank training raises eval Spearman on $|R_{\mathrm{up}}-0.5|$ from $0.705$ (MSE) to $0.729$ (rank) but \emph{reduces} top-20 in-spec count from 13 to 5 vs.\ the same random top-20 baseline of 1 (\texttt{rank\_loss\_comparison.json}).
Holdout $R^2$ therefore favors regression, while Spearman and MEEP outcomes favor the ranking model we actually deploy.
...
\begin{table}[t]
  ...
  \begin{tabular}{ll}
    ...
    SDF mesh audit $R_{\mathrm{up}}$ @ r25--r75 & $\approx 0.500$ on sdf recipe (see \S\ref{sec:mesh}) \\
    ...
    Holdout top-20 in-spec (MSE vs.\ random, $n{=}3695$) & 13 vs.\ 1 \\
    Holdout top-20 in-spec (rank vs.\ random, same split) & 5 vs.\ 1 \\
    Proposal-pool ablation ($n{=}500$, $K{=}20$ fraction) & 15\% vs.\ 10\% (+29\% enrichment) \\
...
Six champions (\texttt{local\_00022}, \texttt{meep\_bo\_00128}, \texttt{meep\_bo\_00093}, \texttt{cand\_000160}, \texttt{cand\_000003}, \texttt{cand\_000261}) pass 6/6 sdf-audit gates: $|R_{\mathrm{up}}(\text{r25}) - R_{\mathrm{up}}(\text{r50})| \le 0.03$ and $|R_{\mathrm{up}}(\text{r25}) - R_{\mathrm{up}}(\text{prod pixel})| \le 0.03$.
On the sdf recipe, $R_{\mathrm{up}}$ at 25--75\,px/$\mu$m clusters near $0.500$ with sub-millipart-per-arm variation (e.g.\ \texttt{local\_00022}: $0.499999$ at r25 sdf vs.\ $0.4994$ on the production pixel grid at r25).
The near-exact $0.500$ readout is \emph{not} a separate physics result: \texttt{phase0\_v1\_sdf\_geom} applies 0.04\,$\mu$m SDF smoothing to the pixel mask, which removes staircase discontinuities that drive resolution-sensitive splits on the raw pixel recipe (left panel of Figure~\ref{fig:mesh}).
Promotion therefore uses sdf cross-checks for stability, while production labels remain on the pixel recipe.
For \texttt{cand\_000261}, $|R_{\mathrm{up}}(\text{r25,sdf}) - R_{\mathrm{up}}(\text{r25,prod})| = 0.005$.
These results support the champion panel on split ratio under the stated contract, not mesh-independent perfection.
...
Rows~6--8 in Table~\ref{tab:surrogate_snapshots} report three diagnostic regimes on the merged corpus (\texttt{surrogate\_validation.json}): random 80/20, $\sigma$-grouped holdout, and chronological train-pre-AL / test-gated-shortlist ($n_{\mathrm{val}}{=}25$).
Under these splits the rank MLP often \emph{loses} to random top-20 on mean absolute error (e.g.\ random-holdout: surrogate mean $|{\mathrm{err}}|{=}0.102$ vs.\ random $0.006$) even when Spearman remains $\sim$0.40--0.46.
That is consistent with val sets enriched for near-50/50 shortlist rows: random draws can land on easy in-spec labels by chance.
These regimes are \textbf{sanity checks}, not the metric used to justify MEEP-gated search.
MEEP shortlist outcomes and sim-budget yield (Table~\ref{tab:budget100}) remain the promotion-facing evidence.
